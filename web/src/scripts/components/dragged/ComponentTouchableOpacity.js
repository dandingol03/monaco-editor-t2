...
const rowSource = {
    beginDrag(props) {
        return {
            index: props.index,
            type: ItemTypes.COMPONENT_TOUCHABLE_OPACITY,
            haff: props.haff,
...
@DragSource(ItemTypes.COMPONENT_TOUCHABLE_OPACITY, rowSource, (connect, monitor) => ({
    connectDragSource: connect.dragSource(),
    isDragging: monitor.isDragging()
}))
class ComponentTouchableOpacity extends Component {
    constructor(props) {
        super(props)
    ...
    render() {
        ...
        var className='Component-TouchableOpacity'
        if(props.selected)
        {
            className+=' clicked'
        ...
        return (
            ...
            connectDragSource(
                <div id={'component-touchable-opacity-' + props.index} className={className} style={defaultStyle} >
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginRight: 5, marginLeft: 5 }}>
                            <span className='icon-touchableOpacity'></span>
                        </div>
                            ...
                            <span className='label'>
                                {
                                    this.props.label !== undefined && this.props.label !== null ?
                                        this.props.label : 'TouchableOpacity'
                                }
                            </span>
                        </div>
                    </div>
...
export default connect()(ComponentTouchableOpacity);
