...
import ListViewInflater from './ListViewInflater'
...
import MemberExpressionParser from '../../../../../public/parser/MemberExpressionParser'
...
class ScrollView extends Component {
    ...
    render() {
        ...
        var { styleMap, device, connectDropTarget, isOver, tabLabel, content , classMethods ,dictionary} = this.props
        //do filering
        ...
        var methodHaff=null
        var listView=null
        //审查子结点是否为表达式,并查找renderRow的具体实现
        if( content && content != '')
        {
            var wrappedReg=/{(.*?)}/
            var methodName=null
            var isContentWrapped=wrappedReg.exec(content)
            if(isContentWrapped!=null&&isContentWrapped[1]!=null)
            {
                var variable=dictionary[isContentWrapped[1]]
                var renderRow=variable.attributes.renderRow
                var ob=renderRow.callee
                while(ob.type!='ThisExpression')
                {
                    methodName=ob.property.name
                    ob=ob.object
                }
                ...
                //覆盖methodName属性
                variable.attributes.renderRow.methodName=methodName
                listView=<ListViewInflater attributes={variable.attributes}/>
            }
        }
        ...
        return (
            ...
            connectDropTarget(<div className={'ScrollView ' + className}
                style={style}
                onClick={(event) => {
                    this.props.onClick();
            ...
            >
                {
                    content && content != '' ?
                        <div ref={(p)=>{
                            ...
                            {/*{content}*/}
                            <div style={{ display: 'flex',  flex: '1 1 auto',flexDirection:'column', justifyContent: 'center' }}>
                                {
                                    listView!=null?
                                    listView:
